...
function isSortudoEligible(message) {
  ...
  const hasD20 =
    roll.dice?.some((d) => d.faces === 20) || roll.formula?.includes("d20");
...
async function handleSortudoClick(message, actor, btnContainer) {
  ...
  if (pm < 3) {
    return ui.notifications.warn(
      `${actor.name} não tem PM suficiente para usar Sortudo (necessário: 3).`,
    );
  }
  ...
  const headerTitle =
    div.querySelector(".card-header h3, .item-name")?.textContent?.trim() ||
    "Teste";
