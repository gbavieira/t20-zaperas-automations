...
export class ItemAutoSaveConfig extends HandlebarsApplicationMixin(ApplicationV2) {
	constructor(options = {}) {
		super(options);
		const raw = game.settings.get(MOD, "itemAutoSavePoisons") ?? [];
		// Normaliza: aceita lista de strings OU lista de objetos {name}
		this.rules = foundry.utils.deepClone(raw).map((r) => (typeof r === "string" ? { name: r } : r));
	}
	...
	_onRender(_context, _options) {
		...
		dropZone.addEventListener("drop", async (ev) => {
			...
			if (item.system?.tipo !== "alchemy") {
				ui.notifications.warn("Apenas itens alquímicos (venenos, bombas, etc.) são aceitos.");
				return;
			}
	...
	static async #resetDefaults(_ev, _target) {
		const confirmed = await DialogV2.confirm({
			...
			content: "<p>Limpar a lista de venenos? Os itens atuais serão <b>perdidos</b>.</p>"
		});
		if (!confirmed) return;
