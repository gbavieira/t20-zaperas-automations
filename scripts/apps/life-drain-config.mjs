		...
		buttons: [
			{
				...
				callback: (_ev, button) => {
					...
					const healPercent = Number(form.querySelector("[name=healPercent]").value);
					const tempHP = form.querySelector("[name=tempHP]").checked;

					if (!spellName) {
						ui.notifications.warn("Arraste uma magia ou poder na zona de drop.");
						return null;
					}
...
export class LifeDrainConfig extends HandlebarsApplicationMixin(ApplicationV2) {
	...
	static async #addSpell(_ev, _target) {
		...
		if (this.rules.some((r) => normalizeText(r.name) === normalizedNew)) {
			ui.notifications.warn(`Já existe uma magia "${newSpell.name}" configurada.`);
			return;
		}
	...
	static async #resetDefaults(_ev, _target) {
		const confirmed = await DialogV2.confirm({
			...
			content: "<p>Restaurar a configuração padrão? As magias atuais serão <b>perdidas</b>.</p>"
		});
		if (!confirmed) return;
